...
Classification of dream affect from EEG signals using the publicly available Dream Emotion Evaluation Dataset (DEED). Two feature extraction methods (PSD and DWT) are compared across two binary classification schemes (Emotional vs. Neutral and Positive vs. Negative) using XGBoost and K-Nearest Neighbours, evaluated with Leave-One-Subject-Out (LOSO) cross-validation and 10-Fold CV. 
